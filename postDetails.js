// postDetails.js

const urlParams = new URLSearchParams(window.location.search);
const id = urlParams.get("postId");

function getPost(){
    axios.get(`${baseUrl}posts/${id}`)
    .then((response) => {
        const post = response.data.data;
        console.log(post);
        const comments =post.comments;
        const author = post.author;
        document.getElementById("username-span").innerHTML = author.username;
        
        let postTitle = "";
        if(post.title != null) {
            postTitle = post.title;
        }
        let commentsContent=``
        for(comment of comments){
            commentsContent +=`
               <!--comment-->
            <div class="p-3 " style="background-color: rgba(218, 213, 213, 0.747);">

            <!--profil pic & user name-->
            <div>
                <img src="${comment.author.profile_image}" class="rounded-circle" style="width: 40px; height: 40px;" alt="">
                <b>${comment.author.username}</b>
            </div>
            <!--// profil pic & user name-->
            <!--comments body-->
            <div>
            ${comment.body}
            </div>
            <!--comments body-->
            <hr/>
            </div>
        
            <!--// comment-->
            `

        }
        const postContent = `
            <div class="card shadow-lg ">
                <div class="card-header">
                    <img class="rounded-circle border border-3" src="${(typeof author.profile_image === 'string' && author.profile_image) ? author.profile_image : 'https://placehold.co/40'}" alt="" style="width: 40px ; height: 40px ; " >
                    <b>${author.username}</b>
                </div>
                <div class="card-body">
                    ${(typeof post.image === 'string' && post.image) ? `<img class="w-100" src="${post.image}" alt="">` : ''}
                    <h6 class="mt-1" style="color: rgb(143, 142, 142);">${post.created_at}</h6>
                    <h5>${postTitle}</h5>
                    <p>${post.body}</p>
                    <hr/>
                    <div>
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pen" viewBox="0 0 16 16">
                                <path d="m13.498.795.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001m-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708z"/>
                            </svg>  
                        <span>(${post.comments_count}) comments</span>
                        
                    </div>
                </div>
                <div id="comments">${commentsContent}   </div>
                <div class="input-group mb-3" id="add-comment-div">
                <input id="comment-input" type="text" placeholder ="add your comment here..." class="form-control p-3" />
                <button class="btn btn-outline-primary" type ="button" onclick="createCommentClicked()">Send</button>
                </div>
            </div>
        `
        document.getElementById("post").innerHTML = postContent;
    });
    console.log('baseUrl:', baseUrl);
console.log('id:', id);
}

function createCommentClicked(){
    const commentBody = document.getElementById("comment-input").value;
    const commenttoken = localStorage.getItem("token");
    const prams = {
        "body": commentBody
    }
    let url = `${baseUrl}posts/${id}/comments`;   
    axios.post(url, prams , {
        headers: {
            "Content-Type": "application/json",
            "Authorization": `Bearer ${commenttoken}`
        }
        })
    .then((response) => {
        getPost();
        document.getElementById("comment-input").value = "";
        showAlert("تم ارسال التعليق بنجاح ✅", "success");
    })
    .catch((error) => {
        const errorMessage = error.response.data.message;
        showAlert(errorMessage, "danger");
    });
}




window.addEventListener('load', function () {
    setupUI();
    if (id) {
        getPost();
    }
});
